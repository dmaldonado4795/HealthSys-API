...
@RestController
@RequestMapping(path = PATH_V1 + "/patient")
...
public class PatientController {
    ...
    public ResponseEntity<Map<String, Object>> getPatients() {
        ...
        if (!patients.isEmpty()) {
            ...
            response.put(ResponseHelper.MESSAGE_KEY, "No registered patients found");
    ...
    public ResponseEntity<Map<String, Object>> getPatientById(@PathVariable long id) {
        ...
        if (patient.isPresent()) {
            ...
            response.put(ResponseHelper.MESSAGE_KEY, "Patient not found");
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(response);
        }
    }
