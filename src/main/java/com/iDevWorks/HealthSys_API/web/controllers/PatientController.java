...
public class PatientController {
    ...
    public ResponseEntity<Map<String, Object>> getPatients() {
        ...
        if (!patients.isEmpty()) {
            resp.put(ResponseHelper.DATA_KEY, patients);
            return ResponseEntity.ok(resp);
        } else {
            resp.put(ResponseHelper.ERROR_KEY, "No registered patients found");
    ...
    public ResponseEntity<Map<String, Object>> getPatientById(@PathVariable long id) {
        ...
        if (patient.isPresent()) {
            resp.put(ResponseHelper.DATA_KEY, patient.get());
            return ResponseEntity.ok(resp);
        } else {
            resp.put(ResponseHelper.ERROR_KEY, "Patient not found");
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(resp);
        }
    }
